...
// Reads records from BAM files and works with and without BAM indexes.
...
impl ReaderOfBams {
    ...
    pub fn load_chr(&mut self, chr : usize) {
        eprintln!("Analyzing {}...", self.chr_names[chr]);
    ...
    pub fn read_next_record( &mut self, file_index: usize, chr: i32, min_pos : usize, max_pos : usize ) -> bool {
        ...
        if self.prev_reads[file_index].is_some() {
            ...
            if tid == chr && pos <= max_pos && pos >= min_pos {
                ...
                // Requested chr does not match saved record, discard saved
                // record and continue reading
